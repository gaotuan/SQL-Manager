...
from core.utils.engines.goinception import GoInceptionEngine
from core.utils.sql_utils import get_syntax_type, remove_comments
from . import EngineBase
from .models import ResultSet, ReviewResult, ReviewSet
from .inception import InceptionEngine
# from core.utils.data_masking import data_masking
...
class MysqlEngine(EngineBase):
    ...
    def query2(self, db_name=None, sql='', limit_num=0, close_conn=True, **kwargs):
        """返回 ResultSet """
        result_set = ResultSet(full_sql=sql)
        data_dict = []
        id = 0
        if kwargs.get('cursorclass'):
        ...
        try:
            ...
            if close_conn:
                self.close()
        return {'data': rows, 'title': data_dict, 'len': len, 'query_time': ''}
